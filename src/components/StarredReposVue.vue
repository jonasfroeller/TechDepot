...
<script lang="ts">
const filteredRepos = ref([] as Repo[])
...
export default {
  ...
  async created() {
    ...
    this.repos = JSON.parse(localStorage.getItem('techdepot-starredrepos') || '[]')

    fetchStarredRepos(username)
      .then((repos) => {
        this.repos = Array.from(repos.values())
        localStorage.setItem('techdepot-starredrepos', JSON.stringify(this.repos))
      })
      .catch((error) => {
        this.error = error
      })

    function filterReposByInput(repos: Repo[], filter: string): Repo[] {
      if (!filter)
        return repos
      ...
      return repos.filter((repo) => {
        return repo.name.toLowerCase().includes(filter.toLowerCase())
          ...
          || (repo.topics && repo.topics.length > 0 && repo.topics.some(topic => topic.toLowerCase().includes(filter.toLowerCase())))
      })
    }

    watchEffect(() => {
      filteredRepos.value = filterReposByInput(this.repos, this.$props?.filter || '')
    })
  },
...
<template>
  <section mb-6 mt-2 p-4 text-center>
    <div v-if="filteredRepos.length > 0" grid grid-cols-1 my-4 gap-4 lg:grid-cols-2 xl:grid-cols-3 :data-amount="filteredRepos.length">
      <div v-for="(repo, index) in filteredRepos" :key="index" border rounded-lg p-4 :data-repo-id="repo.id">
